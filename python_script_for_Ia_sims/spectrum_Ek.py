...
def _ek(field,data):
    return 0.5*(data['x-velocity']*data['x-velocity'] + data['y-velocity']*data['y-velocity'] + data['z-velocity']*data['z-velocity'] )
...
#    nindex_rho = 1./3.
    ...
    Kk = fft_comp(ds, ("gas", "density"), ("gas", "ek"),
...
#      plt.loglog(k, rhomax*(k/kmax)**(-5./3.), ls=":", color="0.5",label="k^(-5/3)")
    ...
    plt.ylabel(r"$Ek (k)$")
    plt.legend(loc=1)
    plt.savefig("Ek_spectrum_"+str(data_num)+".png")
...
yt.add_field('ek',function=_ek,units="km**2/s**2")
num=[1,100,250,350,370,390]
for i in num:
  fn = get_fn(i)
  ds = yt.load(fn)
#  ad = ds.all_data()
#  print ("ek=", ad["kinetic_energy"])
  doit(ds,i)
